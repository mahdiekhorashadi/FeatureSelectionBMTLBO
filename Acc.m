function [E,f,Pc] = Acc(hid,neron,pop,train,n_in,n_out,dim,bias,train_ind,Target,struct1)
...
% Output 1
y1_step1.ymin = -1;
% y1_step1.gain = 2*ones(1,n_out);
% y1_step1.xoffset = 0*ones(1,n_out);
...
% a1 =feval(actFunhidden,(repmat(Bih',1,nT) + w_ih'*x1));
% Hidden Layer
for k = 1:hid-1
...
Pc = n_in*neron+(hid-1)*neron^2+neron*n_out+bias;

f = perform_MSE(Target,y1);
